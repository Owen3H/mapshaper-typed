v0.7.50 (unreleased)
* GeoJSON import uses less memory. The importer started with room for 20,000 coordinates and grew its buffers by half whenever they filled, copying everything read so far each time; importing a large file did this a dozen times or more, and the discarded buffers piled up faster than they could be collected. It now estimates the coordinate count from the file's byte length and reserves the space in one go, as the Shapefile importer already did. The reservation is made when the first path is read, so a layer of points, which does not use these buffers, no longer allocates for them.
