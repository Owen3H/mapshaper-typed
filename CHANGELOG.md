v0.7.50 (unreleased)
* Fixed `-contours` tangling its output on coarse or steep elevation models, where lines crossed each other and spiraled into loops. The smoothing step was allowed to move a line by a whole pixel, but contour lines are only as far apart as the contour interval divided by the slope, which on an 11x11 test file put neighboring lines a tenth of a pixel apart. Smoothing now uses a quarter of a pixel, which still targets the staircase that marching squares leaves (the smoother halves detail at about five times the distance it is given) without moving lines onto their neighbors. Two further safeguards: contour vertices are held a tenth of a cell clear of grid points, so that samples sitting exactly on a contour level -- normal for whole-unit elevations and round levels -- no longer make two branches of a line touch at a point that smoothing would pull into a crossing; and any lines that still cross are put back the way they were traced. The three elevation models tested went from 40, 0 and 346 crossings to none, with 2 of 12,697 lines left unsmoothed on the largest.
* The web UI now shows a raster's own pixels as squares once they are magnified to two screen pixels or more, instead of smoothing them into a blur. Zooming in on a coarse raster shows the data. Smoothing is still used at lower magnifications, and for the scaled-down preview drawn while panning a large raster, whose squares would not be pixels of the raster.
* GeoJSON import uses about a quarter less memory. The importer started with room for 20,000 coordinates and grew its buffers by half whenever they filled, copying everything read so far each time; importing a large file did this fourteen times over, and the discarded buffers piled up faster than they could be collected. It now estimates the coordinate count from the file's byte length and reserves the space in one go, as the Shapefile importer already did, which took a 396MB file from 1379MB of peak memory to 1048MB and a 99MB file from 442MB to 329MB. The reservation is made when the first path is read, so a layer of points, which does not use these buffers, no longer allocates for them.
* GeoParquet and FlatGeobuf export use much less memory. Both converted an entire layer to GeoJSON before encoding it, which for detailed geometry took more memory than the dataset and the output file put together; they now convert and encode a piece at a time. Writing a 133MB GeoParquet went from 2.9GB to 1.5GB of peak memory and a 184MB FlatGeobuf from 2.3GB to 1.6GB, both around a third faster. The output files are byte-for-byte unchanged.
...
* Fixed GeoParquet output that some readers could not open at all. When a row group's bounding box had a coordinate on a whole number, which rounded, snapped or integer-grid data makes likely, that bound was written to the row group statistics as an integer where the Parquet spec requires a double, and the file's footer then failed to deserialize in pyarrow 21+ and GDAL 3.11+. Such bounds are now widened by the smallest amount a double can express, pending a fix in the hyparquet-writer library.
